...
export const ALLOWED_SETTING_KEYS = ['edinet_api_key', 'anthropic_api_key'] as const;
export type SettingKey = (typeof ALLOWED_SETTING_KEYS)[number];

/** 暗号化して保存すべき機密キー（現状は全キーが機密だが、将来の非機密設定に備えて分離） */
const SECRET_SETTING_KEYS: ReadonlySet<string> = new Set(['edinet_api_key', 'anthropic_api_key']);
...
export async function saveUserSetting(
  ...
  const storedValue = SECRET_SETTING_KEYS.has(key) ? encryptSetting(value) : value;
