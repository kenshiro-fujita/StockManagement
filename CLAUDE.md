...
### 開発スタイル（最重要）
- **小さく切り出す**: 変数・定数・関数・モジュールはできる限り小さく切り出し、アジャイルな変更に対応できるようにする。1つの関数は1つの責務に絞る。
- **コメントを書く**: 人間の認知負荷を下げるため、変数・定数・関数などの意図や目的を説明するコメントを必ずつける。「何をしているか」ではなく「なぜそうしているか」を書く。
